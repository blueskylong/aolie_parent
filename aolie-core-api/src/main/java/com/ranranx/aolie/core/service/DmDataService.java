...
public class DmDataService extends LocalDmDataService {


    @DubboReference
    protected UiService uiService;
    @DubboReference
    protected FixRowService rowService;

    @Autowired
    protected FixRowDataService rowDataService;
    ...
    private HandleResult initFixBlockData(QueryParam queryParam) {
        ...
        if (criteriaFix.isEmpty() || hasFixColFilter(queryParam)) {
            ...
            if (rowService.checkNeedFixBlock(mapValue, queryParam.getTable())) {
                return rowDataService.copyFixTableRow(mapValue, queryParam.getTable(), queryParam
                );
    ...
    public HandleResult updateLevel(Map<Long, String> mapIdToCode, long viewId) {
        BlockViewer viewerInfo = uiService.getViewerInfo(viewId, SessionUtils.getLoginVersion());
