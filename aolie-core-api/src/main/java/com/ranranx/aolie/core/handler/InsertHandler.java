...
import com.ranranx.aolie.common.exceptions.UnknownException;
...
public class InsertHandler<T extends InsertParam> extends BaseHandler<T> {
    ...
    private List<List<Long>> genKeys(InsertParam param) {
        ...
        try {
            ...
            for (Map<String, Object> row : param.getLstRows()) {
                lstChangeId.add(genAndUpdateKey(row, tableId, keyField, errTable, param.isNeedEmptyIdField()));
            }
            return lstChangeId;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @return
     */
    private List<Long> genAndUpdateKey(Map<String, Object> row, Long tableId, String keyField,
                                       String errTable, boolean isNeedEmptyIdField) {
        Object obj = row.get(keyField);
        if (isNeedEmptyIdField && obj != null && (!CommonUtils.isNumber(obj.toString())
                || Long.parseLong(obj.toString()) >= 0)) {
            throw new InvalidParamException(errTable + " 指定的值不符合预期,预期为一负数,或为空");
        }
        long nextId;
        //如果原来就指定了值,则不再生成
        if (!isNeedEmptyIdField && obj != null && Long.parseLong(obj.toString()) > 0) {
            nextId = Long.parseLong(obj.toString());
        } else {
            nextId = IdGenerator.getNextId("table_" + SessionUtils.getLoginVersion() + "_" + tableId);
        }
