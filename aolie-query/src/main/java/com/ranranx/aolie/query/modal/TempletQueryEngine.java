...
import com.ranranx.aolie.common.types.DbType;
...
import com.ranranx.aolie.common.tree.TreeFactory;
...
public class TempletQueryEngine {
    ...
    private String genFilter(Map<String, Object> filter, String tableAlias, int[] paramIndex,
        ...
        boolean isOracle = CommonUtils.getDefaultDbType().equals(DbType.ORACLE);
        ...
        filter.forEach((key, value) -> {
            ...
            if (isOracle) {
                sbSql.append(tableAlias).append(".").append(key).append(" like '%'||")
                        .append(value.toString().replaceAll("'", "''")).append("||'%') and ");
            } else {
                sbSql.append(tableAlias).append(".").append(key).append(" like concat(concat('%',")
    ...
    private String createAndInsertTemptable(
        ...
        if (CommonUtils.getDefaultDbType().equals(DbType.ORACLE)) {
            sbSql.append("create global TEMPORARY table  ").append(tempTableName).append("  as ");
        } else {
...
//        }
    ...
    private List<Map<String, Object>> queryDetailData(Node queryStruct, List<VerticalSeparatorInfo> lstColInfo
        ...
        if (isOracle) {
            ...
            sbSql.delete(sbSql.length() - 2, sbSql.length()).append(" as ").append(VerticalSeparatorInfo.FIELD_XH).append(",");
        } else {
            sbSql.append("concat(");
            for (String preField : preFields) {
                sbSql.append(tableAliasB).append(".").append(preField).append(" ,").append("'_',");
