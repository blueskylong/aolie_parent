...
import com.ranranx.aolie.common.types.*;
...
@DubboService
public class DataModelService implements IDmService {
    Logger logger = LoggerFactory.getLogger(DataModelService.class);
    ...
    private IReferenceDataFilter dataFilter;
    ...
    private List<ReferenceData> findReferenceDataByFilter(long referenceId, String version, Criteria extFilter) {
        Reference reference = SchemaService.getReference(referenceId, version);
        if (reference == null) {
            return null;
        }
    ...
    public List<ReferenceData> findColumnReferenceData(long referenceId, long colId,
        ...
        Column column = SchemaService.getColumn(colId, version);
        if (column == null) {
    ...
    private List<ReferenceData> findColumnReferenceDataByService(long referenceId, long colId,
        ...
        IReferenceDataFilter service = ApplicationService.getService(serviceName, IReferenceDataFilter.class);
        if (service == null) {
            throw new NotExistException("指定的服务名不存在:" + serviceName);
        }
        IReferenceDataFilter filter = service;
    ...
    public List<String> findDefaultDBTablesNotInSchema(Long schemaId, String version) {
        if (factory.getDataOperatorBySchema(schemaId, version).getDbType().equals(DbType.ORACLE)) {
    ...
    private List<String> findDefaultDBTablesNotInSchemaForOracle(Long schemaId, String version) {
        ...
        List<String> lstViews = findDefaultDBViewsNotInSchemaForOracle(schemaId, version);
        if (lstViews != null && !lstViews.isEmpty()) {
            lst.addAll(lstViews);
        }
        return lst;
    }

    private List<String> findDefaultDBViewsNotInSchemaForOracle(Long schemaId, String version) {
        QueryParamDefinition definition = new QueryParamDefinition();
        definition.setTableNames("user_views");
        List<Field> lstField = new ArrayList<>();
        Field field = new Field();
        field.setFieldName("view_name");
        field.setTableName("user_views");
        lstField.add(field);
        definition.setFields(lstField);
        List<String> schemaTableNames = findSchemaTableNames(schemaId, version);
        if (!schemaTableNames.isEmpty()) {
            ...
            definition.getSingleCriteria().andNotIn("user_views", "view_name", schemaTableNames);
        }
        Criteria criteria = definition.getSingleCriteria();
        if (EXCLUDE_TABLE_PREFIX != null) {
            for (int i = 0; i < EXCLUDE_TABLE_PREFIX.length; i++) {
                criteria.andNotStartWith("user_views", "view_name", EXCLUDE_TABLE_PREFIX[i].toUpperCase());
            }
        }
        ...
        List<String> lst = new ArrayList<>();
        for (Map<String, Object> map : lstData) {
            lst.add(map.get("view_name").toString().toLowerCase());
        }
        return lst;
    }
    ...
    private List<String> findSchemaTableNames(long schemaId, String version) {
        ...
        HandleResult handleResult = handlerFactory.handleQuery(new QueryParam()
                .setFilterObjectAndTableAndResultType(schemaId, version, dto));
        List<TableDto> lstDto = handleResult.getData();
        if (lstDto == null || lstDto.isEmpty()) {
            return new ArrayList<>();
        }
        int length = lstDto.size();
        List<String> table = new ArrayList<>(length);
        for (int i = 0; i < length; i++) {
            table.add(lstDto.get(i).getTableName());
        }
        return table;
    ...
    public List<ColumnDto> findTableFieldAsColumnDto(String tableName) {
        ...
        if (factory.getDefaultDataOperator().getDbType().equals(DbType.ORACLE)) {
    ...
    private List<ColumnDto> syncTableColumnCommon(long tableId, String version, boolean isNeedCommit) {
        TableInfo table = SchemaService.getTable(tableId, version);
        if (table == null) {
            return null;
        }
