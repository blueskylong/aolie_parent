...
public class UIServiceImpl implements UiService {
    ...
    public List<Map<String, Object>> findTablesAndFields(long[] tableIds) {
        ...
        for (long tableId : tableIds) {
            TableInfo tableInfo = SchemaService.getTable(tableId, version);
            if (tableInfo == null) {
    ...
    private TableInfo findViewDsInfo(Component com) {
        return SchemaService.getTable(com.getColumn().getColumnDto().getTableId(), com.getComponentDto().getVersionCode());
    }
    ...
    private List<Component> findViewerComponents(Long blockViewId, String version) {
        ...
        for (ComponentDto dto : lstDto) {
            Column column = SchemaService.getColumn(dto.getColumnId(), dto.getVersionCode());
            if (column == null) {
    ...
    public Map<String, List<Long>> getViewServiceNameRelation() {
        ...
        queryParamDefinition.setTableDtos(BlockViewDto.class);
        queryParamDefinition.appendCriteria()
                .andEqualTo(null, "version_code", SessionUtils.getDefaultVersion());
        List<BlockViewDto> lstView = factory.getDefaultDataOperator().select(queryParamDefinition, BlockViewDto.class);

        Map<String, List<Long>> result = new HashMap<>();
        if (lstView == null || lstView.isEmpty()) {
            return result;
        }
        lstView.forEach(blockViewDto -> {
            Long schemaId = blockViewDto.getSchemaId();
            Schema schema = SchemaHolderLocal.getSchema(schemaId, SessionUtils.getDefaultVersion());
            String applicationCode = schema.getSchemaDto().getApplicationCode();
            List<Long> lstLong = result.computeIfAbsent(applicationCode, key -> {
                return new ArrayList<>();
            });
            if(blockViewDto.getBlockViewId() == 5110968078920192L){
                System.out.println();
            }
            lstLong.add(blockViewDto.getBlockViewId());
        });
        return result;
    }
