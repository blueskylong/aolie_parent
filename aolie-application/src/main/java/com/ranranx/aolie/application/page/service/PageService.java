...
public class PageService {
    ...
    public List<Long> findPageRefTables(long pageId) {
        ...
        for (PageDetailDto dto : lstPageDetail) {
            ...
            if (dto.getViewType() == Constants.PageViewType.reference) {
                ...
                BlockViewer viewerInfo = uiService.getViewerInfo(dto.getViewId(), dto.getVersionCode());
    ...
    public void savePageFullInfo(PageInfo pageInfo) {
        ...
        dmDataService.saveSlaveRowsByObject(pageInfo.getLstPageDetail(), PageDetailDto.class, PageInfoDto.class,
    ...
    public List<Map<String, Object>> getPageElements(Long schemaId) {
        ...
        List<Reference> referenceDtos = SchemaHolder.getReferences(SessionUtils.getLoginVersion());
     ...
     * @param result
     */
    private void addReferenceInfo(List<Reference> lstReferenceDto, List<Map<String, Object>> result) {
        Map<String, Object> map = new HashMap<>();
        map.put("id", 12121);
        map.put("code", "002");
        map.put("name", "引用");
        ...
        if (lstReferenceDto != null && !lstReferenceDto.isEmpty()) {
            for (Reference dto : lstReferenceDto) {
                map = new HashMap<>();
                map.put("id", dto.getReferenceDto().getRefId());
                map.put("code", "002" + provider.getNextCode());
                map.put("name", dto.getReferenceDto().getRefName());
                map.put("type", "2");
                result.add(map);
            }
        }
